...
class Player:
    ...
    def check(self, drawPile, discardPile, playerChoice, newGame):
        ...
        if playerChoice == 0:
            ...
            draw_card = drawPile[0]
            drawPile.pop(0)
            self.plDeck.insert(0, draw_card)
            print('inserted')
            print(draw_card)

            if self.check(drawPile, discardPile, 1, newGame) is None:
                print('cant play')
            else:
                print('can play')
                # print("in if")
    ...
    def playTurn(self, newGame, drawPile, discardPile, screen):
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        ...
                        seventh_card_rect = pygame.transform.smoothscale(card_interface[j], (100, 150)).get_rect(
                            topleft=(900, 500))
                        Eight_card_rect = pygame.transform.smoothscale(card_interface[j], (100, 150)).get_rect(
                            topleft=(950, 500))
                        ninth_card_rect = pygame.transform.smoothscale(card_interface[j], (100, 150)).get_rect(
                            topleft=(1000, 500))
        ...
        draw_new_card_rect = pygame.transform.smoothscale(image_small, (100, 150)).get_rect(topleft=(750, 280))
                    ...
                    for event in pygame.event.get():
                        ...
                        if event.type == pygame.MOUSEBUTTONDOWN:
                            ...
                            if draw_new_card_rect.collidepoint(x, y):
                                playerChoice = 0
                                screen.blit(pygame.transform.smoothscale(drawPile[0].cardimage, (100, 150)), (550, 280))
                                pygame.display.update()
                                self.check(drawPile, discardPile, playerChoice, newGame)
                                time.sleep(3)
                                rectCoord = [100, 150, 575, 280]
